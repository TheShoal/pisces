...
2. **Local shared gateway** (default path)
   - Uses a single shared process coordinated under `~/.pisces/agent/python-gateway`.
...
6. Load extension modules from:
   - user: `~/.pisces/agent/modules/*.py`
   - project: `<cwd>/.pisces/modules/*.py` (overrides same-name user module)
...
2. Managed venv at `~/.pisces/python-env`
...
- custom MIME `application/x-pisces-status` -> structured status events
...
- `application/x-pisces-status` -> status events

...
```text
~/.pisces/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.pisces/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.pisces/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.pisces/agent/sessions`

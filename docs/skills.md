...
- `description` is required for:
  - native `.pisces` provider skill discovery (`requireDescription: true`)
...
1. `native` (priority 100) — `.pisces` user/project skills via `src/discovery/builtin.ts`
...
- source toggles: `enableCodexUser`, `enableClaudeUser`, `enableClaudeProject`, `enablePiscesUser`, `enablePiscesProject`

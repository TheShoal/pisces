...
| `PI_SUBPROCESS_CMD`        | Overrides subagent spawn command (`pisces` / `pisces.cmd` resolution bypass)                       |
...
| Variable | Default / behavior |
|---|---|
| `PI_CONFIG_DIR` | Config root dirname under home (default `.pisces`) |
| `PI_CODING_AGENT_DIR` | Full override for agent directory (default `~/<PI_CONFIG_DIR or .pisces>/agent`) |

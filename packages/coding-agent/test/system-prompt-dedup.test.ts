...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".pisces");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".pisces"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".pisces", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".pisces", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".pisces", "SYSTEM.md"), "Project SYSTEM prompt");

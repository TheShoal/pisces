/**
 * Discovery integration tests for Pisces plugin registry reading.
 *
 ...
 * depends on:
 *   1. The Pisces registry lives at path.join(home, ".pisces", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".pisces")
...
// but we know the value is ".pisces" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".pisces";
...
let tmpHome: string;
/** ~/.pisces/plugins/installed_plugins.json inside tmpHome */
let ompRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "pisces-discovery-test-"));
...
describe("Pisces registry path contract", () => {
	it("Pisces registry lives at home/.pisces/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".pisces", "plugins", "installed_plugins.json");
		expect(ompRegistryPath).toBe(expected);
	});

	it("Pisces config dir name is .pisces", () => {
		...
		expect(OMP_CONFIG_DIR).toBe(".pisces");
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("Pisces registry format compatibility with Claude parser", () => {
...
describe("Pisces registry round-trip", () => {
...
//
// listClaudePluginRoots must replace Claude entries with Pisces entries when the same
...
describe("Pisces precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — Pisces entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// Pisces entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const ompEntry = makeEntry("/pisces/cached/path");

		// Pisces registry entry has installPath (required by listClaudePluginRoots)
